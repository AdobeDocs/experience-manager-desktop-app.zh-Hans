...
* **Adobe Experience Manager实例**：提供对AEM Assets DAM存储库中存储的资源的访问权限。 此外，它代表与挂载的网络共享交互的本地桌面应用程序执行桌面应用程序请求的操作。 目标AEM实例应运行AEM版本6.1或更高版本。 运行早期AEM版本的AEM实例可能需要安装额外的功能包和修补程序才能完全正常工作。
...
* AEM Desktop将在资源文件类型的默认编辑器中打开资源。
* 用户对资源进行所需的更改。
* 修改文件后，用户可以使用AEM Desktop的后台同步状态窗口查看文件的同步状态。
* 使用AEM桌面机的上下文菜单，用户可签入/签出资产，或返回到DAM用户界面。
* 完成对文件的更改后，用户返回到AEM Web UI

此方案不是唯一的用例。 但是，它说明了AEM Desktop如何成为一种便于在本地访问/编辑资源的机制。 我们建议您尽可能多地使用DAM Web UI，因为它提供了更好的体验。 它为Adobe提供了更大的灵活性以满足客户需求。
...
Adobe建议在将某些类型的文件能够有效地从已装载目录就地编辑的提交给客户端之前，先执行一些用例测试。

AEM Desktop不适合执行密集型文件系统操作，包括但不限于：
...
不会将每个操作缓存在本地。 以下内容将立即传输到AEM服务器，而无需本地缓存：
...
当您在本地与文件交互时，AEM Desktop会检查AEM中是否提供了较新版本的文件。 如果有新版本可用，应用程序会将文件的新副本下载到本地缓存。 但是，如果本地缓存的文件经过修改，AEM Desktop不会覆盖该文件。 此功能可防止意外覆盖您的工作。

在本地和AEM中修改同一文件时，本地修改的版本将覆盖AEM中的版本。 在这种情况下，资产时间轴中提供了先前版本。 您可以验证两个版本并解决任何冲突。

如果本地文件与服务器中可用的版本不一致，则后台上载状态对话框将通知您有关冲突的信息。 要解决此问题，请打开发生冲突的文件并保存该文件。 保存文件将强制AEM Desktop将您的最新本地更改同步到AEM。 您可以在时间轴中查看资产的早期版本并解决任何冲突。

当多个用户尝试在针对同一AEM实例的单独装入目录中工作时，请考虑其他因素。 特别是，以下因素很重要：

* 用户原始网络上的可用带宽量
* 原始网络的网络配置，如防火墙或代理
* 目标AEM实例网络中的可用带宽量
* Dispatcher是否显示在目标AEM实例之前
* 目标AEM实例上的当前负载
...
另一种提高AEM性能的方法是配置Granite临时工作流队列作业的最大并行作业值。 建议值大约为服务器可用CPU数的一半。 要调整值，请执行以下步骤：
...
* 在针对大型客户端进行故障诊断时，Adobe将其AEM实例的大小配置为c4.8xlarge，主要用于4000 Mbps的专用带宽。
...
在写入签出的文件时，仅在AEM WebDAV实施中强制实施锁定。 因此，使用WebDAV的客户端（如桌面应用程序）只强制执行锁定。 锁定不会通过AEM Web界面强制执行。 AEM界面只会在已签出的资产的卡片视图中显示一个锁定图标。 图标是修饰性的，不会对AEM的行为产生影响。

通常，WebDAV客户端并不总是按预期运行。 可能还有其他问题。 但是，在AEM中刷新或检查资源是验证资源是否未被修改的可靠方法。 这种行为是典型的OS WebDAV客户端行为，该客户端不受Adobe控制。
...
替换文件不会显示警告或错误，但检查AEM中的资源会显示它保持不变。 在AEM中刷新或检查资源以验证其未被修改。
...
AEM Desktop会尝试同步任何给定文件三次。 如果第三次尝试后文件无法同步，AEM Desktop会将该文件视为冲突，并通过后台上传状态窗口通知您。 冲突状态表示您可在本地使用最新更改，但这些更改不会同步回AEM。 AEM桌面应用程序不再尝试同步。

解决这种情况的最简单方法是打开冲突文件并再次保存。 它会强制AEM Desktop另外尝试进行三次同步。 如果文件仍无法同步，请参阅以下部分以获取更多帮助。

## 清除AEM桌面缓存 {#clearing-aem-desktop-cache}

清除AEM Desktop的缓存是一项初步的故障排除任务，可以解决几个AEM Desktop问题。
...
但是，根据AEM Desktop配置的AEM端点，该位置可能会发生更改。 该值是目标URL的编码版本。 例如，如果应用程序面向`http://localhost:4502`，则目录名称为`http%3A%2F%2Flocalhost%3A4502%2F`。

要清除缓存，请删除&lt;Encoded AEM Endpoint>目录。

>[!NOTE]
>
>如果清除AEM桌面缓存，并且本地文件更改未同步到AEM，则会丢失这些更改。
...
在macOS上升级AEM桌面应用程序时，有时可能会出现问题。 AEM桌面应用程序的旧版系统文件夹导致了这些问题。 它可阻止正确加载AEM桌面的新版本。 要解决此问题，可以手动删除以下文件夹和文件。
...
1. 验证`/content/dam.json`之前的所有内容是否与AEM Desktop中配置的目标AEM值相匹配。
...
AEM Desktop可与Dispatcher后面的AEM部署配合使用，后者是AEM服务器的默认和推荐的配置。 AEM创作环境前的AEM Dispatcher通常配置为跳过缓存DAM资源。 因此，调度程序不会从AEM Desktop角度提供其他缓存。 确保已调整Dispatcher配置以使其适用于AEM桌面版。 有关其他详细信息，请参阅[使用Dispatcher连接到AEM](install-configure-app-v1.md#connect-to-an-aem-instance-behind-a-dispatcher)。

## 检查日志文件 {#checking-for-log-files}

根据您的操作系统，您可以在以下位置找到AEM桌面版的日志文件：
