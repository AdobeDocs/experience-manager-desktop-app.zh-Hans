...
[!DNL Adobe Experience Manager]桌面应用程序将您的数字资产管理(DAM)解决方案链接到桌面，以便您可以直接在桌面上打开AEM Web UI中提供的文件。 如果您从桌面上保存了资产，则该资产会上传到相应位置的AEM。
...
>[!NOTE]
>
>在阅读本文档之前，您可以查看整个[AEM和Creative Cloud集成最佳实践](https://experienceleague.adobe.com/zh-hans/docs/experience-manager-65/content/assets/administer/aem-cc-integration-best-practices)，以了解该主题的更高级概述。
...
当文件被保存时，额外的写缓存确保它首先被存储在本地，使用户能够避免等待网络传输。 然后，在预定义的延迟（30秒）后，文件将在后台上传到AEM，然后将资产上传到AEM。 AEM桌面应用程序提供了一个用于监控后台文件上传状态的UI。
...
* **保存对文件的更改**。 您保存到网络共享中文件的任何更改都将自动上传到AEM，并创建一个新版本。
...
* **正在批量上传大型分层文件夹**。 如果您使用AEM UI中的&#x200B;**创建** > **文件夹上传**&#x200B;选项来上传资产，AEM桌面应用程序会将选定的文件夹层次结构上传到后台的AEM。 使用桌面应用程序中的专用UI监控上传进度。

## 不当使用AEM桌面应用程序 {#inappropriate-use-of-aem-desktop-app}

* 请勿使用AEM桌面应用程序从桌面管理资源。 AEM桌面应用程序未构建为网络驱动器的替代产品。 请改用以下功能：

   * 用于数字资源管理（查找或共享资源、元数据以及复制或移动）的AEM Assets Web UI。

   * AEM桌面应用程序[!UICONTROL Folder Upload]以上传大型分层文件夹。

* 请勿将AEM桌面应用程序视为AEM Assets的“桌面同步”客户端。 AEM桌面应用程序的主要优势在于，它提供了对整个存储库的“虚拟”访问，并且桌面同步应用程序通常只同步属于一个用户的资产。 AEM桌面应用程序提供了一定级别的缓存和后台上传；尽管如此，它的工作方式与典型的“同步”应用程序非常不同，例如Adobe Creative Cloud桌面应用程序或Microsoft OneDrive。

* 请勿使用AEM桌面应用程序网络驱动器频繁保存资产。 所有保存操作都将传输到AEM Assets。 因此，直接在装入的AEM Assets存储库中执行大量编辑操作是不切实际的。 直接在挂载的存储库中编辑资产会用不相关的版本阻塞资产的时间线，并在服务器上施加额外的开销。

* 请勿使用AEM桌面应用程序将大量数据从一个AEM实例迁移到另一个实例。 请参阅[迁移指南](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/assets-migration-guide)以规划和执行资产迁移。 相反，桌面应用程序[支持在[!DNL Adobe Experience Manager]中首次批量上传](use-app-v1.md#bulkupload)大量资产。

## 针对选定用例的建议 {#recommendations-for-selected-use-cases}

### 创意用户访问资产 {#access-to-assets-for-creative-users}

AEM桌面应用程序提供对整个DAM存储库的虚拟访问 — 对于桌面上的创意用户而言，在其桌面上查找并访问正确的资源可能会比较复杂。 请使用这些最佳实践来简化操作。

* 使用AEM Assets Web UI中的协作功能，创意用户能够更直接地访问正确的资源。 共享文件夹或收藏集、提供智能收藏集（保存的搜索）或发送包含指向正确资源的指针的通知就是其中之一。 然后，Creative用户可以使用Web UI中的桌面操作快速访问其桌面上的这些资源。
...
* 考虑对资产（访问控制）的正确权限，以便为创意用户简化DAM存储库中的视图，从而基本上限制他们仅访问所需或感兴趣的资产：
   ...
   * 只有需要更改或修饰的资产才应为创意用户启用写功能。 某些组织使用AEM项目及其创建的文件夹来托管仍可能发生更改的资源。
...
* 找到资产后，可使用AEM UI中的桌面操作在桌面上访问该资产。

### 更新使用AEM桌面应用程序打开的资源 {#updating-assets-opened-using-aem-desktop-app}

如果您在从AEM Assets映射到本地网络共享的位置直接编辑资源，则每次将资源保存到桌面上时，都会将该资源上传到AEM。 此外，AEM会创建一个版本并生成节目。
...
若要了解应该如何优化AEM Assets服务器以提高性能，请转到[AEM Assets性能优化指南](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/performance-tuning-guidelines)。 AEM桌面应用程序服务器性能的一些重要方面与优化工作流配置有关，以便它在资源上传时运行良好：

* **更多性能资产上传**。 将[AEM资产更新工作流模型配置为临时](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/assets/administer/performance-tuning-guidelines)。

* **限制服务器CPU的上传**。 请确保正确设置了最大并行工作流作业参数，以便上传不会耗尽所有CPU。
