---
cloud: Experience Cloud
product: adobe experience manager
audience: end-user
user-guide-title: Experience Manager 桌面应用程序指南
breadcrumb-title: Desktop 应用程序指南
user-guide-description: 了解如何使用 Experience Manager 桌面应用程序连接存储库和桌面程序，以便更快地访问资源并简化工作流。
feature-set: Experience Manager Assets,Experience Manager
feature: Desktop App
role: User, Admin
source-git-commit: cde31207ddb0da1057db15f3fcbeec4dd3d0cd8c
workflow-type: tm+mt
source-wordcount: '85'
ht-degree: 48%

---


# Adobe Experience Manager桌面应用程序指南 {#using}

+ [概述](introduction.md)
+ [发行说明](release-notes.md)
+ [安装或升级](install-upgrade.md)
+ [开始使用AEM桌面应用程序](get-started.md)
+ [用户界面](user-interface.md)
+ [搜索](search.md)
+ [使用桌面应用程序](using-desktop-app.md)
+ [Assets管理](assets-management-tasks.md)
+ [下载资产](download-assets.md)
+ [上传资产](upload-assets.md)
+ [使用多个资产](working-with-multiple-assets.md)
+ [疑难解答](troubleshoot.md)
+ 以前版本的帮助 {#archive}
   + [发行说明v1](release-notes-of-v1.md)
   + [安装和配置v1](install-configure-app-v1.md)
   + [使用应用程序v1](use-app-v1.md)
   + [最佳实践v1](best-practices-for-v1.md)
   + [故障诊断v1](troubleshoot-app-v1.md)
